...
VCF_FILE := $(SAMPLE_NAME)/vcf/$(SAMPLE_NAME).vcf.gz
VCF_INDEX := $(SAMPLE_NAME)/vcf/$(SAMPLE_NAME).vcf.gz.tbi
# =========================================
USE_SRA ?= no 
...
> @echo "  READ_LIMIT can be set to limit the number of threads downloaded (for testing)"
...
# =========================================
# Convert BAM to VCF using bcftools mpileup and call
#mpileup -> call -> norm -> sort -> index
toVCF: $(VCF_FILE)
$(VCF_FILE): $(ALIGNMENT_BAM) $(REFSEQ_FASTA)
> bcftools mpileup -Ou -f $(REFSEQ_FASTA) $< | \
	bcftools call -mv -Ou | \
	bcftools norm -f $(REFSEQ_FASTA) -Ou | \
	bcftools sort -Oz -o $@
  #bgzip the files
.PHONY: toVCF
# =========================================
...
# =========================================
# Index VCF file using bcftools index
VCFindex: $(VCF_INDEX)
$(VCF_INDEX): $(VCF_FILE)
> tabix -p vcf $<
.PHONY: VCFindex
